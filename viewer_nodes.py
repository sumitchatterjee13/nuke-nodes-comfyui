...
class NukeViewer(NukeNodeBase):
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "image": ("IMAGE",),
                "channel": (["rgba", "rgb", "red", "green", "blue", "alpha", "luminance"], {"default": "rgba"}),
                "gamma": ("FLOAT", {"default": 1.0, "min": 0.1, "max": 3.0, "step": 0.1}),
                "gain": ("FLOAT", {"default": 1.0, "min": 0.1, "max": 5.0, "step": 0.1}),
    ...
    def view(self, image, channel, gamma, gain, show_overlay, overlay_text, mask=None):
        ...
        if img_processed.shape[3] >= 4:
            ...
            r = img_processed[:, :, :, 0:1]
            g = img_processed[:, :, :, 1:2] if img_processed.shape[3] > 1 else torch.zeros_like(r)
            b = img_processed[:, :, :, 2:3] if img_processed.shape[3] > 2 else torch.zeros_like(r)
            a = torch.ones_like(r)
        ...
        if channel == "rgba":
            if img_processed.shape[3] >= 4:
                ...
                result = torch.cat([img_processed, torch.ones_like(img_processed[:, :, :, :1])], dim=3)
        elif channel == "rgb":
        ...
        if mask is not None and show_overlay:
            ...
            if mask.shape[1:3] != result.shape[1:3]:
                mask = F.interpolate(mask.permute(0, 3, 1, 2), size=result.shape[1:3], mode='bilinear', align_corners=False).permute(0, 2, 3, 1)
            
            # Create mask overlay (red tint where mask is active)
            mask_alpha = mask[:, :, :, :1]
            overlay_color = torch.tensor([1.0, 0.0, 0.0], device=result.device).view(1, 1, 1, 3)
...
class NukeChannelShuffle(NukeNodeBase):
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "image": ("IMAGE",),
                "red_from": (["red", "green", "blue", "alpha", "zero", "one"], {"default": "red"}),
                "green_from": (["red", "green", "blue", "alpha", "zero", "one"], {"default": "green"}),
                "blue_from": (["red", "green", "blue", "alpha", "zero", "one"], {"default": "blue"}),
                "alpha_from": (["red", "green", "blue", "alpha", "zero", "one"], {"default": "alpha"}),
            }
        }
    ...
    def shuffle(self, image, red_from, green_from, blue_from, alpha_from):
        ...
        if img.shape[3] >= 4:
            ...
            channels = {
                "red": img[:, :, :, 0:1],
                "green": img[:, :, :, 1:2] if img.shape[3] > 1 else torch.zeros_like(img[:, :, :, :1]),
                "blue": img[:, :, :, 2:3] if img.shape[3] > 2 else torch.zeros_like(img[:, :, :, :1]),
...
class NukeRamp(NukeNodeBase):
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "ramp_type": (["horizontal", "vertical", "radial", "diagonal", "checkerboard"], {"default": "horizontal"}),
    ...
    def generate(self, width, height, ramp_type, color_start, color_end, invert, batch_size):
        """
        ...
        if ramp_type == "horizontal":
            ...
            y_centered = torch.arange(height, dtype=torch.float32).view(-1, 1) - center_y
            x_centered = torch.arange(width, dtype=torch.float32).view(1, -1) - center_x
            distance = torch.sqrt(x_centered**2 + y_centered**2)
            max_distance = torch.sqrt(torch.tensor(center_x**2 + center_y**2, dtype=torch.float32))
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "width": ("INT", {"default": 512, "min": 64, "max": 4096, "step": 1}),
                "height": ("INT", {"default": 512, "min": 64, "max": 4096, "step": 1}),
                "pattern": (["smpte", "rgb_bars", "primary_colors", "grayscale"], {"default": "smpte"}),
                "batch_size": ("INT", {"default": 1, "min": 1, "max": 64, "step": 1}),
            }
        }
