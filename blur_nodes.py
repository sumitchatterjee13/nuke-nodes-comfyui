...
class NukeBlur(NukeNodeBase):
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "image": ("IMAGE",),
                "size_x": ("FLOAT", {"default": 1.0, "min": 0.0, "max": 100.0, "step": 0.1}),
                "size_y": ("FLOAT", {"default": 1.0, "min": 0.0, "max": 100.0, "step": 0.1}),
                "filter": (["gaussian", "box", "triangle", "quadratic"], {"default": "gaussian"}),
    ...
    def blur(self, image, size_x, size_y, filter, quality, crop, mix, mask=None):
        ...
        if mask is not None:
            ...
            if mask.shape[1:3] != blurred.shape[1:3]:
                mask = F.interpolate(mask.permute(0, 3, 1, 2), size=blurred.shape[1:3], mode='bilinear', align_corners=False).permute(0, 2, 3, 1)
    ...
    def _gaussian_blur(self, img_tensor, size_x, size_y, quality):
        ...
        if size_x > 0 and kernel_size_x > 1:
            ...
            kernel_x = self._create_gaussian_kernel(kernel_size_x, sigma_x, img_tensor.device)
        ...
        if size_y > 0 and kernel_size_y > 1:
            sigma_y = size_y / 3.0
            kernel_y = self._create_gaussian_kernel(kernel_size_y, sigma_y, img_tensor.device)
    ...
    def _box_blur(self, img_tensor, size_x, size_y):
        ...
        if size_x > 0:
            ...
            kernel_x = torch.ones(1, 1, 1, kernel_size_x, device=img_tensor.device) / kernel_size_x
        ...
        if size_y > 0:
            ...
            kernel_y = torch.ones(1, 1, kernel_size_y, 1, device=img_tensor.device) / kernel_size_y
    ...
    def _triangle_blur(self, img_tensor, size_x, size_y):
        ...
        if size_x > 0:
            ...
            coords = torch.arange(kernel_size_x, dtype=torch.float32, device=img_tensor.device)
        ...
        if size_y > 0:
            ...
            coords = torch.arange(kernel_size_y, dtype=torch.float32, device=img_tensor.device)
...
class NukeMotionBlur(NukeNodeBase):
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "image": ("IMAGE",),
                "distance": ("FLOAT", {"default": 10.0, "min": 0.0, "max": 100.0, "step": 0.1}),
                ...
                "center_bias": ("FLOAT", {"default": 0.0, "min": -1.0, "max": 1.0, "step": 0.01}),
                "mix": ("FLOAT", {"default": 1.0, "min": 0.0, "max": 1.0, "step": 0.01}),
            }
        }
    ...
    def motion_blur(self, image, distance, angle, samples, shutter, center_bias, mix):
        ...
        for i in range(samples):
            ...
            grid = self._create_motion_grid(offset_x, offset_y, height, width, img_tensor.device)
            
            # Sample image
            sample = F.grid_sample(img_tensor, grid, mode='bilinear', align_corners=False)
...
class NukeDefocus(NukeNodeBase):
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "image": ("IMAGE",),
                "defocus": ("FLOAT", {"default": 1.0, "min": 0.0, "max": 20.0, "step": 0.1}),
                "aspect_ratio": ("FLOAT", {"default": 1.0, "min": 0.1, "max": 3.0, "step": 0.01}),
                "quality": (["low", "medium", "high"], {"default": "medium"}),
    ...
    def defocus(self, image, defocus, aspect_ratio, quality, method, mix, depth_map=None, focus_distance=0.5):
        """
        ...
        if depth_map is not None:
            ...
            if depth.shape[1:3] != img.shape[1:3]:
                depth = F.interpolate(depth.permute(0, 3, 1, 2), size=img.shape[1:3], mode='bilinear', align_corners=False).permute(0, 2, 3, 1)
        ...
        # Apply variable blur
        result = self._apply_variable_blur(img, blur_amount, aspect_ratio, quality, method)
    ...
    def _apply_variable_blur(self, img, blur_amount, aspect_ratio, quality, method):
        ...
        if method == "gaussian":
            ...
            if kernel_size > 1:
                sigma = max_blur / 3.0
                kernel = self._create_gaussian_kernel_2d(kernel_size, sigma, sigma/aspect_ratio, img_tensor.device)
                kernel = kernel.unsqueeze(0).unsqueeze(0).repeat(img_tensor.shape[1], 1, 1, 1)
            ...
            for _ in range(passes):
                ...
                if kernel_size > 1:
                    sigma_x = size_x / 2.0
                    sigma_y = size_y / 2.0
                    kernel = self._create_gaussian_kernel_2d(kernel_size, sigma_x, sigma_y, img_tensor.device)
                    kernel = kernel.unsqueeze(0).unsqueeze(0).repeat(result.shape[1], 1, 1, 1)
    ...
    def _create_gaussian_kernel_2d(self, size, sigma_x, sigma_y, device):
        ...
        kernel = torch.exp(-(x_grid ** 2) / (2 * sigma_x ** 2) - (y_grid ** 2) / (2 * sigma_y ** 2))
        kernel = kernel / kernel.sum()
